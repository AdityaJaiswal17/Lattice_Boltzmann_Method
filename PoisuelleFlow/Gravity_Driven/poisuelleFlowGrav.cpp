...
int main()
{
    ...
    for(int i=0; i<LX; i++) //collison
    {
        for(int j=1; j<LY-1; j++)
        {
            for(int k=0; k<zeta; k++)
            {
                double Forceterm = (1.0 - (1.0/2.0)*(tauinv))*Wlist[k]*(3.0*(Cx[k]-u_x[i][j]) + 4.5*((Cx[k]*u_x[i][j] + Cy[k]*u_y[i][j])*Cx[k]))*density[i][j]*grav;
                ftemp[i][j][k] = f[i][j][k]*(1-tauinv) + f_eq[i][j][k]*(tauinv) + Forceterm;
            }
        }
    }
    ...
    for(int i=0; i<LX; i++)
    {
        for(int j=1; j<LY-1; j++)
        {
            ...
            density[i][j] = tempDensity;
            u_x[i][j] = (tempUx/density[i][j]) + (grav/2.0);
